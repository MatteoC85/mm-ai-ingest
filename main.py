...
# Retrieval v6 arbiter (keep high-level ask/root-cause logic untouched; arbitrate retrieval only)
RETRIEVAL_V6_ARBITER_ENABLED = (os.environ.get("MM_RETRIEVAL_V6_ARBITER_ENABLED") or "1").strip() != "0"
RETRIEVAL_V6_ASK_MARGIN = float(os.environ.get("MM_RETRIEVAL_V6_ASK_MARGIN", "0.10"))
RETRIEVAL_V6_DIAGNOSTIC_MARGIN = float(os.environ.get("MM_RETRIEVAL_V6_DIAGNOSTIC_MARGIN", "0.03"))
RETRIEVAL_V6_MERGE_MARGIN = float(os.environ.get("MM_RETRIEVAL_V6_MERGE_MARGIN", "0.06"))
RETRIEVAL_V6_CORE_TOP_BONUS = float(os.environ.get("MM_RETRIEVAL_V6_CORE_TOP_BONUS", "0.12"))
RETRIEVAL_V6_SUPPORT_TOP_PENALTY = float(os.environ.get("MM_RETRIEVAL_V6_SUPPORT_TOP_PENALTY", "0.18"))
RETRIEVAL_V6_SUPPORT_ONLY_HARD_FAIL = (os.environ.get("MM_RETRIEVAL_V6_SUPPORT_ONLY_HARD_FAIL") or "1").strip() != "0"
RETRIEVAL_V6_INSTALL_FALSE_POSITIVE_HARD_FAIL = (os.environ.get("MM_RETRIEVAL_V6_INSTALL_FALSE_POSITIVE_HARD_FAIL") or "1").strip() != "0"
RETRIEVAL_V6_DEBUG = (os.environ.get("MM_RETRIEVAL_V6_DEBUG") or "0").strip() == "1"
...
def _dense_candidates_multi_query(
    *,
    ...
    if use_role_surrogate:
        embed_inputs = [_build_dense_query_embedding_surrogate(q) for q in cleaned_queries]
    else:
        embed_inputs = [re.sub(r"\s+", " ", _normalize_unicode_advanced(q or "")).strip() for q in cleaned_queries]
...
def _build_chunk_embedding_surrogate(
    ...
    *,
    ...
    # Do not over-amplify installation/positioning chunks in embeddings.
    if _looks_like_installation_positioning_false_positive(text) or role_class == "support_startup_install":
        surrogate = "\n".join(prefix_parts + [text]).strip()
        return surrogate[:RETRIEVAL_SURROGATE_MAX_CHARS]

    if role_class in {"core_process", "core_mechanical"}:
        prefix_parts.append(f"retrieval role: {role_class}")
        if tags:
            prefix_parts.append("retrieval tags: " + ", ".join(tags[:10]))
    elif role_class == "support_electrical_interlock":
        prefix_parts.append("retrieval role: support_electrical_interlock")
        keep_tags = [t for t in tags if t in {"support_electrical_interlock", "electrical_control", "safety_interlock", "forming", "material_feed", "drive_train"}]
        if keep_tags:
            prefix_parts.append("retrieval tags: " + ", ".join(keep_tags[:8]))
    elif role_class == "support_safety":
        prefix_parts.append("retrieval role: support_safety")
        keep_tags = [t for t in tags if t in {"support_safety", "safety_interlock"}]
        if keep_tags:
            prefix_parts.append("retrieval tags: " + ", ".join(keep_tags[:6]))
    elif role_class == "support_lubrication":
        prefix_parts.append("retrieval role: support_lubrication")
        keep_tags = [t for t in tags if t in {"support_lubrication", "lubrication", "drive_train", "material_feed", "forming"}]
        if keep_tags:
            prefix_parts.append("retrieval tags: " + ", ".join(keep_tags[:6]))
    elif role_class != "collateral":
        prefix_parts.append(f"retrieval context: {role_class}")
...
def _shared_semantic_retrieval_v5_core(
    *,
    q: str,
    company_id: str,
    machine_id: str,
    ...
    use_role_surrogate = not (_retrieval_mode_family(planner_mode) == "ask" and not diagnostic_mode)
    ...
    chunks_matching_filter, candidates, query_vectors = _dense_candidates_multi_query(
        ...
        bubble_document_id=bubble_document_id,
        debug=debug,
        use_role_surrogate=use_role_surrogate,
    )
    ...
    lexical_candidates: list[dict] = []
    enable_role_aware_lexical_blend = RETRIEVAL_V5_ENABLED and (
        diagnostic_mode
        or _retrieval_mode_family(planner_mode) != "ask"
        or (not candidates and _query_symptom_profile(q).get("classes"))
    )
    if enable_role_aware_lexical_blend and (RETRIEVAL_LEXICAL_ALWAYS_BLEND or not candidates or _query_symptom_profile(q).get("classes")):
...
def _retrieval_mode_family(planner_mode: str) -> str:
    pm = str(planner_mode or "").strip().lower()
    if pm.startswith("ask"):
        return "ask"
    if "root_cause" in pm or pm == "draft_ps":
        return "diagnostic"
    return "general"


def _role_payload_from_candidate(item: dict) -> dict:
    role = item.get("queryless_role") or {}
    role_class = str(role.get("role_class") or item.get("role_class_hint") or item.get("role_class") or "").strip()
    if role_class:
        role = dict(role)
        role.setdefault("role_class", role_class)
        role.setdefault("role_group", str(role.get("role_group") or item.get("role_group_hint") or item.get("role_group") or ("support" if role_class.startswith("support_") else ("core" if role_class.startswith("core_") else "collateral"))))
        role.setdefault("role_tags", list(role.get("role_tags") or []))
        return role

    chunk_text = (item.get("chunk_full") or item.get("snippet") or "").strip()
    source_type = str(item.get("source_type") or _source_type_from_document_id(item.get("bubble_document_id") or ""))
    return _queryless_chunk_role_signature(chunk_text, source_type=source_type)


def _annotate_retrieval_roles(items: list[dict]) -> list[dict]:
    out: list[dict] = []
    for item in items or []:
        cc = dict(item)
        role = _role_payload_from_candidate(cc)
        cc.setdefault("queryless_role", role)
        cc.setdefault("role_class_hint", str(role.get("role_class") or "collateral"))
        cc.setdefault("role_group_hint", str(role.get("role_group") or "collateral"))
        cc.setdefault("source_type", str(cc.get("source_type") or _source_type_from_document_id(cc.get("bubble_document_id") or "")))
        out.append(cc)
    return out


def _retrieval_arbiter_proxy_score(
    q: str,
    retrieval: dict,
    *,
    planner_mode: str,
    ...
    citations = _annotate_retrieval_roles(list((retrieval or {}).get("citations") or []))
    if not citations:
        return {
            "score": -1.0,
            "hard_fail": True,
    ...
    mode_family = _retrieval_mode_family(planner_mode)
    profile = _query_symptom_profile(q)
    classes = set(profile.get("classes") or [])

    score = float(_retrieval_quality_score({**retrieval, "citations": citations}) or -1.0)
    notes: list[str] = []
    hard_fail = False

    top = citations[0]
    top_role = dict(top.get("queryless_role") or {})
    top_role_class = str(top_role.get("role_class") or "collateral")
    top_role_group = str(top_role.get("role_group") or "collateral")
    top_text = (top.get("chunk_full") or top.get("snippet") or "").strip()

    top3 = citations[:3]
    role_groups = [str((c.get("queryless_role") or {}).get("role_group") or c.get("role_group_hint") or "collateral") for c in top3]
    core_count = sum(1 for g in role_groups if g == "core")
    support_count = sum(1 for g in role_groups if g == "support")
    family_count = len({str(_stable_evidence_family_key(c)) for c in top3 if c.get("citation_id")})
    lexical_strong = sum(1 for c in top3 if set(str(x).strip().lower() for x in (c.get("lexical_match_types") or []) if str(x).strip()) & {"phrase", "exact"})

    if lexical_strong:
        score += min(0.06, 0.02 * lexical_strong)

    if classes & {"vibration", "noise", "jam"}:
        if _looks_like_installation_positioning_false_positive(top_text):
            score -= 0.42
            notes.append("top_installation_positioning_false_positive")
            if mode_family == "ask" and RETRIEVAL_V6_INSTALL_FALSE_POSITIVE_HARD_FAIL:
                hard_fail = True

        if profile.get("generic_symptom"):
            if top_role_group == "core":
                score += RETRIEVAL_V6_CORE_TOP_BONUS
                notes.append("core_top_bonus")
            else:
                score -= RETRIEVAL_V6_SUPPORT_TOP_PENALTY
                notes.append("support_top_penalty")

            if top_role_class == "support_startup_install":
                score -= 0.22
                notes.append("startup_install_top_penalty")
                if mode_family == "ask" and RETRIEVAL_V6_SUPPORT_ONLY_HARD_FAIL:
                    hard_fail = True
            if top_role_class in {"support_lubrication", "support_safety", "support_electrical_interlock", "support_fluid_power"} and not profile.get("has_support_anchor"):
                score -= 0.10
                notes.append("generic_support_secondary_penalty")
            if core_count > 0:
                score += 0.05 + 0.02 * min(core_count, 2)
                notes.append("core_presence_bonus")
            if core_count == 0 and support_count >= min(2, len(top3)):
                score -= 0.16
                notes.append("support_only_top3_penalty")
                if mode_family == "ask" and RETRIEVAL_V6_SUPPORT_ONLY_HARD_FAIL:
                    hard_fail = True

        if profile.get("has_bending_anchor") and any(("forming" in set(str(x).strip() for x in ((c.get("queryless_role") or {}).get("role_tags") or [])) or str((c.get("queryless_role") or {}).get("role_class") or "") == "core_process") for c in top3):
            score += 0.06
            notes.append("bending_anchor_bonus")
        if profile.get("has_feed_anchor") and any((set(str(x).strip() for x in ((c.get("queryless_role") or {}).get("role_tags") or [])) & {"material_feed", "straightening"} or str((c.get("queryless_role") or {}).get("role_class") or "") == "core_process") for c in top3):
            score += 0.06
            notes.append("feed_anchor_bonus")

    if "no_start" in classes:
        if profile.get("automatic_mode"):
            if top_role_class in {"support_electrical_interlock", "support_safety"}:
                score += 0.12
                notes.append("automatic_interlock_bonus")
            if top_role_class == "support_startup_install":
                score -= 0.14
                notes.append("automatic_startup_install_penalty")
            if top_role_class == "support_lubrication" and not profile.get("has_support_anchor"):
                score -= 0.16
                notes.append("automatic_lubrication_penalty")
        else:
            if top_role_class == "support_electrical_interlock":
                score += 0.10
                notes.append("no_start_electrical_bonus")
            if top_role_class == "support_startup_install":
                score -= 0.08
                notes.append("no_start_install_penalty")

    if diagnostic_mode and family_count >= 2:
        score += min(0.05, 0.02 * family_count)
        notes.append("diagnostic_diversity_bonus")
...
def _merge_retrieval_candidate_pools(*pools: list[dict]) -> list[dict]:
    by_id: dict[str, dict] = {}
    for pool in pools:
        for item in pool or []:
            cid = str(item.get("citation_id") or "").strip()
            ...
            if prev is None:
                by_id[cid] = cc
                continue
            prev_key = _candidate_order_key(prev)
            cur_key = _candidate_order_key(cc)
            if cur_key < prev_key:
                merged = dict(prev)
                merged.update(cc)
                by_id[cid] = merged
...
def _build_merged_retrieval_result(
    q: str,
    *,
    legacy: dict,
    candidate: dict,
    top_k: int,
    diagnostic_mode: bool,
    chosen_planner: Optional[dict] = None,
) -> dict:
    merged = dict(candidate if float((candidate or {}).get("similarity_max") or 0.0) >= float((legacy or {}).get("similarity_max") or 0.0) else legacy)
    merged_candidates = _merge_retrieval_candidate_pools(
        list((candidate or {}).get("candidates") or []),
        list((legacy or {}).get("candidates") or []),
        list((candidate or {}).get("citations") or []),
        list((legacy or {}).get("citations") or []),
    )
    seed = _dedup_citations_by_snippet(
        list((candidate or {}).get("citations") or []) + list((legacy or {}).get("citations") or []),
        max_items=max(top_k + 3, ROOT_CAUSE_MAX_EVIDENCE_POOL if diagnostic_mode else top_k + 1),
    )
    locked = _lock_final_citations(
        selected_citations=seed,
        ranked_candidates=merged_candidates + seed,
        top_k=top_k,
        diagnostic_mode=diagnostic_mode,
        query_token_count=_count_query_tokens(q),
    )
    merged["planner"] = chosen_planner or merged.get("planner") or {}
    merged["candidates"] = merged_candidates
    merged["citations"] = locked or seed[:top_k]
    merged["chunks_matching_filter"] = max(
        int((legacy or {}).get("chunks_matching_filter") or 0),
        int((candidate or {}).get("chunks_matching_filter") or 0),
    ) or ((legacy or {}).get("chunks_matching_filter") if (legacy or {}).get("chunks_matching_filter") is not None else (candidate or {}).get("chunks_matching_filter"))
    merged["similarity_max"] = max(float((legacy or {}).get("similarity_max") or 0.0), float((candidate or {}).get("similarity_max") or 0.0))
    merged["effective_threshold"] = min(float((legacy or {}).get("effective_threshold") or ASK_SIM_THRESHOLD), float((candidate or {}).get("effective_threshold") or ASK_SIM_THRESHOLD))
    merged["fts_used"] = bool((legacy or {}).get("fts_used")) or bool((candidate or {}).get("fts_used"))
    merged["prefix_fts_used"] = bool((legacy or {}).get("prefix_fts_used")) or bool((candidate or {}).get("prefix_fts_used"))
    merged["exact_fts_used"] = bool((legacy or {}).get("exact_fts_used")) or bool((candidate or {}).get("exact_fts_used"))
    if "phrase_fts_used" in legacy or "phrase_fts_used" in candidate:
        merged["phrase_fts_used"] = bool((legacy or {}).get("phrase_fts_used")) or bool((candidate or {}).get("phrase_fts_used"))
    merged["rerank_used"] = bool((legacy or {}).get("rerank_used")) or bool((candidate or {}).get("rerank_used"))
    merged["rerank_error"] = (candidate or {}).get("rerank_error") or (legacy or {}).get("rerank_error")
    return merged


def _shared_semantic_retrieval_legacy_core(
    *,
    q: str,
    company_id: str,
    machine_id: str,
...
def _shared_semantic_retrieval(
    *,
    ...
    if not RETRIEVAL_V6_ARBITER_ENABLED:
        return _shared_semantic_retrieval_v5_core(
            q=q,
            company_id=company_id,
            machine_id=machine_id,
            candidate_k=candidate_k,
    ...
    legacy = _shared_semantic_retrieval_legacy_core(
        q=q,
        company_id=company_id,
        machine_id=machine_id,
        candidate_k=candidate_k,
    ...
    if not RETRIEVAL_V5_ENABLED:
        legacy["retrieval_arbiter"] = {
            "legacy_score": _retrieval_arbiter_proxy_score(q, legacy, planner_mode=planner_mode, diagnostic_mode=diagnostic_mode),
            "candidate_score": None,
            "chosen": "legacy",
        }
        return legacy

    candidate = _shared_semantic_retrieval_v5_core(
        q=q,
        company_id=company_id,
        machine_id=machine_id,
        candidate_k=candidate_k,
    ...
    legacy_eval = _retrieval_arbiter_proxy_score(q, legacy, planner_mode=planner_mode, diagnostic_mode=diagnostic_mode)
    candidate_eval = _retrieval_arbiter_proxy_score(q, candidate, planner_mode=planner_mode, diagnostic_mode=diagnostic_mode)

    mode_family = _retrieval_mode_family(planner_mode)
    chosen_name = "legacy"
    chosen = legacy

    if mode_family == "ask" and not diagnostic_mode:
        if legacy_eval["hard_fail"] and not candidate_eval["hard_fail"]:
            chosen = candidate
            chosen_name = "candidate"
        elif candidate_eval["hard_fail"] and not legacy_eval["hard_fail"]:
            chosen = legacy
            chosen_name = "legacy"
        elif candidate_eval["score"] >= legacy_eval["score"] + RETRIEVAL_V6_ASK_MARGIN:
            chosen = candidate
            chosen_name = "candidate"
        else:
            chosen = legacy
            chosen_name = "legacy"
    elif mode_family == "ask" and diagnostic_mode:
        if legacy_eval["hard_fail"] and not candidate_eval["hard_fail"]:
            chosen = candidate
            chosen_name = "candidate"
        elif candidate_eval["hard_fail"] and not legacy_eval["hard_fail"]:
            chosen = legacy
            chosen_name = "legacy"
        elif candidate_eval["score"] >= legacy_eval["score"] + RETRIEVAL_V6_DIAGNOSTIC_MARGIN:
            chosen = candidate
            chosen_name = "candidate"
        elif abs(candidate_eval["score"] - legacy_eval["score"]) <= RETRIEVAL_V6_MERGE_MARGIN and not candidate_eval["hard_fail"] and not legacy_eval["hard_fail"]:
            chosen = _build_merged_retrieval_result(
                q, legacy=legacy, candidate=candidate, top_k=top_k, diagnostic_mode=diagnostic_mode, chosen_planner=candidate.get("planner") or legacy.get("planner")
            )
            chosen_name = "merged"
        else:
            chosen = legacy if legacy_eval["score"] >= candidate_eval["score"] else candidate
            chosen_name = "legacy" if chosen is legacy else "candidate"
    else:
        if candidate_eval["hard_fail"] and not legacy_eval["hard_fail"]:
            chosen = legacy
            chosen_name = "legacy"
        elif legacy_eval["hard_fail"] and not candidate_eval["hard_fail"]:
            chosen = candidate
            chosen_name = "candidate"
        elif candidate_eval["score"] >= legacy_eval["score"] + RETRIEVAL_V6_DIAGNOSTIC_MARGIN:
            if diagnostic_mode and abs(candidate_eval["score"] - legacy_eval["score"]) <= RETRIEVAL_V6_MERGE_MARGIN and legacy_eval["core_count"] > 0 and candidate_eval["core_count"] > 0:
                chosen = _build_merged_retrieval_result(
                    q, legacy=legacy, candidate=candidate, top_k=top_k, diagnostic_mode=diagnostic_mode, chosen_planner=candidate.get("planner") or legacy.get("planner")
                )
                chosen_name = "merged"
                ...
                chosen = candidate
                chosen_name = "candidate"
        elif legacy_eval["score"] > candidate_eval["score"] + RETRIEVAL_V6_DIAGNOSTIC_MARGIN:
            chosen = legacy
            chosen_name = "legacy"
        elif diagnostic_mode and not legacy_eval["hard_fail"] and not candidate_eval["hard_fail"] and (candidate_eval["family_count"] != legacy_eval["family_count"] or candidate_eval["core_count"] != legacy_eval["core_count"]):
            chosen = _build_merged_retrieval_result(
                q, legacy=legacy, candidate=candidate, top_k=top_k, diagnostic_mode=diagnostic_mode, chosen_planner=candidate.get("planner") or legacy.get("planner")
            )
            chosen_name = "merged"
        else:
            chosen = candidate if candidate_eval["score"] >= legacy_eval["score"] else legacy
            chosen_name = "candidate" if chosen is candidate else "legacy"

    chosen = dict(chosen)
    chosen["retrieval_arbiter"] = {
        "legacy_score": legacy_eval,
        "candidate_score": candidate_eval,
        "chosen": chosen_name,
    ...
    def _finalize(resp: dict) -> dict:
        if payload.debug:
            resp["debug"] = {
                ...
                "retrieval_arbiter": retrieval.get("retrieval_arbiter"),
